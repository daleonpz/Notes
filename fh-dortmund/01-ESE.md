...
- Refinement:
    ...
    - for any action "a", if exist a transtition in the refinement given the action "a"  ( s -(a)> A (s1) ) , then it should be a transition in the original model for given the same action "a" ( t -(a)> B (t1) ). Note that the neither the source state nor the destiny state shouldn't be the same for both models. 
